...
1. `api.ts`: методы принимают `targetUserId?: string` и кладут его в body только если задан (не слать `undefined` — envelope/requestHash должен совпадать со старым при отсутствии).
